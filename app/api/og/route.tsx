...
async function loadFontFromPublic(req: Request) {
  try {
    // baue absolute URL auf deinen public/ Pfad
...
export async function GET(req: Request) {
  ...
  // Font aus /public laden (HTTP, nicht Dateisystem)
  const fontData = await loadFontFromPublic(req)

  return new ImageResponse(
    (
      <div style={{ height: "100%", width: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", background: "#0e0e0f", padding: 48, position: "relative" }}>
        {/* Glow */}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", color: "white", fontSize: 28, opacity: 0.85 }}>
    ...
    ),
    {
      ...
      fonts: fontData
        ...
        : [], // Fallback: ohne Font weiter rendern
    }
  )
}
